...
async function main() {
    switch (command) {
        ...
        case "player": {
            const addr = rest[0];
            if (!addr) {
                console.error("Usage: player <address>");
                process.exit(1);
            }
        ...
        case "enter": {
            ...
            if (!shellAmt) {
                console.error("Usage: enter <shell-amount>");
                process.exit(1);
            }
        ...
        case "deposit": {
            const shellAmt = rest[0];
            if (!shellAmt) {
                console.error("Usage: deposit <shell-amount>");
                process.exit(1);
            }
        ...
        case "withdraw": {
            ...
            if (!krillAmt) {
                console.error("Usage: withdraw <krill-amount>");
                process.exit(1);
            }
        ...
        case "purge": {
            const addr = rest[0];
            if (!addr) {
                console.error("Usage: purge <address>");
                process.exit(1);
            }
        ...
        case "settle-delinquent": {
            const addr = rest[0];
            if (!addr) {
                console.error("Usage: settle-delinquent <address>");
                process.exit(1);
            }
        ...
        case "set-tax-rate": {
            const rate = rest[0];
            if (!rate) {
                console.error("Usage: set-tax-rate <rate-in-krill-per-block>");
                process.exit(1);
            }
        ...
        case "distribute-to-address": {
            const [to, amt] = rest;
            if (!to || !amt) {
                console.error("Usage: distribute-to-address <address> <krill-amount>");
                process.exit(1);
            }
            const result = await gameCore.distributeToAddress(to, parseKrill(amt));
            json(result);
            break;
        }
        case "distribute-to-all": {
            const amt = rest[0];
            if (!amt) {
                console.error("Usage: distribute-to-all <krill-amount>");
                process.exit(1);
            }
        ...
        case "distribute-to-voters": {
            const amt = rest[0];
            if (!amt) {
                console.error("Usage: distribute-to-voters <krill-amount>");
                process.exit(1);
            }
        ...
        case "start-campaign": {
            const bribe = rest[0];
            if (!bribe) {
                console.error("Usage: start-campaign <bribe-per-vote-in-krill>");
                process.exit(1);
            }
        ...
        case "fund-campaign": {
            const amt = rest[0];
            if (!amt) {
                console.error("Usage: fund-campaign <krill-amount>");
                process.exit(1);
            }
        ...
        case "update-bribe": {
            const bribe = rest[0];
            if (!bribe) {
                console.error("Usage: update-bribe <new-bribe-per-vote-in-krill>");
                process.exit(1);
            }
        ...
        case "vote": {
            ...
            if (!candidate) {
                console.error("Usage: vote <candidate-address>");
                process.exit(1);
            }
        ...
        case "reclaim-funds": {
            const term = rest[0];
            if (!term) {
                console.error("Usage: reclaim-funds <term-number>");
                process.exit(1);
            }
